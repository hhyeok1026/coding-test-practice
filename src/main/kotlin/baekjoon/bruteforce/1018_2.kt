...
작은걸로 픽해야하는구나..
=> 함수로 빼서 함수를 2번 돌리게 했고, 코드가 길어져서 변수명, 퇴고, 주석제거를 해야하는데, 이 코드를 다시 볼일 없을테니 귀찮아서 그냥 놔둬야겠음.
...
fun main(args: Array<String>) {
    ...
    var minFixNum : Int = 32 // 모두 고쳐야할 경우는 32개이다. (?)
    // 2. 자르는 로직이 필요함
    ...
    for (baseRowNum in 0 until n) {
        ...
        for (baseColumnNum in 0 until m) {
            ...
            val whiteStartFixNum = getFixNumForStartColor('W', baseRowNum, baseColumnNum, boardState)
            val blackStartFixNum = getFixNumForStartColor('B', baseRowNum, baseColumnNum, boardState)

            val nowFixNum = if (whiteStartFixNum < blackStartFixNum) {
                whiteStartFixNum
            } else {
                blackStartFixNum
            }
